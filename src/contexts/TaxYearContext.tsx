...
import { TaxYear, TaxYearConfig, StateConfig, YearStatus, VersionSnapshot, WorkflowGates } from '@/types/tax';
...
interface TaxYearContextType {
  ...
  finalizeYear: (gates: WorkflowGates) => boolean;
  lockYear: () => boolean;
...
export function TaxYearProvider({ children }: { children: ReactNode }) {
  ...
  const persisted = loadFromStorage<PersistedYearState>(STORAGE_KEYS.TAX_YEAR);

  const [currentYear, setCurrentYearState] = useState<TaxYear | null>(
    persisted?.currentYear ?? null
  );
  const [yearConfig, setYearConfig] = useState<TaxYearConfig | null>(
    persisted?.yearConfig ?? null
  );
  const availableYears = generateAvailableYears();

  // ── Persist whenever year or config changes ───────────────────────────────
  useEffect(() => {
    saveToStorage(STORAGE_KEYS.TAX_YEAR, { currentYear, yearConfig });
  ...
  const finalizeYear = useCallback((gates: WorkflowGates): boolean => {
    if (!yearConfig || yearConfig.isLocked) return false;
    if (yearConfig.states.length === 0) return false;
    if (!passesFederalAndFinalizationGates(gates)) return false;
